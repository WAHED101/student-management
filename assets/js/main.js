...
(function(){
  ...
  window.flatpickr(dobInput, {
    ...
    onReady: (selectedDates, dateStr, instance) => {
      ...
      if (alt) {
        ...
        if (originalPlaceholder) alt.setAttribute('placeholder', originalPlaceholder);
      }
  ...
  function convertYearToDropdown(instance){
    ...
    if (!select){
      ...
      select.style.height = yearInput.offsetHeight ? yearInput.offsetHeight + 'px' : '';
      // Build options
      ...
      yearInput.parentNode && yearInput.parentNode.replaceChild(select, yearInput);
...
(function(){
  ...
  function updateControls(index){
    ...
    if (btnSubmit){
      ...
      btnSubmit.disabled = index === steps.length - 1 ? !isStepFilled(index) : false;
    }
  }
  ...
  function setStep(index){
    ...
    if (goingForward && index-1 >= 0 && lines[index-1]){
      ...
      setTimeout(()=>{ targetLine.classList.remove('loading'); }, 700);
    }
    current = index;
  ...
  btnPrev && btnPrev.addEventListener('click', ()=> setStep(Math.max(0, current-1)));
